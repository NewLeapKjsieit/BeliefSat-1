...
#define image_quality 5  //try changing this from 0-7 and see which quality suits the speed
...
void setup()
{
    ...
    radio.setDio1Action(receive_interrupt);
    if(flag.silent_mode)
...
void loop()
{
    ...
    switch (currentmode)
    {
        case normal_mode:
        {
            ...
            {wdt_reset();telemetry();wdt_reset();last_telemetry=millis();break;}
        }
        case safe_mode:
        {
        ...
        {
            if(abs(last_telemetry-millis())>=30000)
            {wdt_reset();telemetry();wdt_reset();last_telemetry=millis();}
            if(abs(last_ssdv-millis())>=(600000)
            {wdt_reset();transmit_ssdv();wdt_reset();last_ssdv=millis();break;}
        }
    }
}
...
void process_cmd()
{
    ...
    int j=decode_rs_8(major_part,0,0,223-16);if(j<0){return;}//discard uplink if the correction fails
    int n=memcmp(&major_part[7],callsign_bytes,6);
    if(n!=0){return;} //check if the uplink is for the same satellite if not then discard
    switch(major_part[0])//switch as per the packet packet_type
    {
        ...
        case 0xEF: //if it is internal command
        {
            ...
            {
                while(1){} //make an infinite loop so that watchdog resets the MCU 
            }
            ...
            {
                if(currentmode==safe_mode){return;}
...
void transmit_ssdv()
{
    ...
    uint8_t ssdv_packet[256], image_buffer[64]; bool done=false;
    static ssdv_t ssdv;int r;uint8_t temp = 0, temp_last = 0;
